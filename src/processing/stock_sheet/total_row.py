...
from src.processing.stock_sheet.constants import empty_canonical_row, enforce_canonical_row
...
def maybe_repair_total_row(
    *,
    ...
    extract_tables: Callable[[bytes], list[list[list[str]]]],
) -> dict[str, Any] | None:
    ...
    early_tables = extract_tables(_crop(page_image, EARLY_CROP_LEFT, EARLY_CROP_RIGHT))
    tail_tables = extract_tables(_crop(page_image, TAIL_CROP_LEFT, TAIL_CROP_RIGHT))
