...
from src.models.api.packaging_list import PackagingListContainerInfo
...
def align_containers_to_packaging_list(
    bill_containers: list[BillOfLadingContainerRow],
    packaging_containers: list[PackagingListContainerInfo],
    fuzzy_threshold: float = 0.85,
) -> list[BillOfLadingContainerRow]:
    """
    Build one output B/L container row per packaging-list container.

    The packaging list is the canonical brand-specific container set. B/L vision
    extraction can skip a row or misread a single character, so this function:
      1. matches each packaging container to an unused B/L row,
      2. returns the packaging-list container number as the canonical output value,
      3. uses the B/L pkg_ct when matched, and
      4. falls back to packaging-list no_of_bags when the B/L row was skipped.
    """
    if not packaging_containers:
        logger.warning("No packaging container info provided, returning all bill containers")
        return bill_containers

    aligned: list[BillOfLadingContainerRow] = []
    matched_bill_indices: set[int] = set()

    for pkg in packaging_containers:
        pkg_no = pkg.container_number
        remaining_bills = [
            c for i, c in enumerate(bill_containers) if i not in matched_bill_indices
        ]
        ...
        if match:
            ...
            matched_bill_indices.add(idx)
            if match.container_no != pkg_no:
                logger.info(
                    f"Canonicalized container number from bill={match.container_no} "
                    f"to packaging={pkg_no} ({match_type}, score={score:.3f})"
                )
            aligned.append(
        ...
        if pkg.no_of_bags is not None:
            logger.warning(
                f"B/L extraction missed packaging container {pkg_no}; "
                f"using packaging no_of_bags={pkg.no_of_bags}"
            )
            aligned.append(
                BillOfLadingContainerRow(
                    container_no=pkg_no,
                    pkg_ct=pkg.no_of_bags,
                )
            )
        else:
            logger.warning(
                f"B/L extraction missed packaging container {pkg_no} and "
                "packaging no_of_bags is unavailable; row omitted"
            )

    logger.info(
        f"Container alignment complete: {len(aligned)}/{len(packaging_containers)} "
        f"packaging containers represented"
    )
    return aligned
